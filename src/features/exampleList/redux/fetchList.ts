...
// uses redux-thunk for async actions by default: https://github.com/gaearon/redux-thunk
...
export function useFetchRedditList() {
  ...
  const { redditList, fetchRedditListPending, fetchRedditListError } = useSelector(
    (state: RootState) => ({
      redditList: state.list.redditList,
      fetchRedditListPending: state.list.fetchRedditListPending,
      fetchRedditListError: state.list.fetchRedditListError,
    }),
    shallowEqual,
  );
